...
When probing the Transposition Table at the start of any node search, the retrieved `bestMove` is immediately passed to the move ordering subsystem, receiving the absolute maximum score (`10,000,000`). This ensures that the best move from previous iterations is searched first, producing instant beta cutoffs across transposition branches.
...
Pawn structure calculations are computationally intensive. ADAM offloads this to a dedicated 16,384-entry direct-mapped `PawnTable` (`Engine/evaluation/pawn_table.h`):
- Indexed by `pawnKey`.
...
ADAM's search engine operates inside `Engine/search/search.cpp` using a fail-soft Alpha-Beta framework.
...
ADAM includes automated testing scripts in the root directory: `run_test.bat` and `run_test.ps1`.
...
The `Engine/test/` directory provides dedicated test runners:
